...
def setup_train_args():
    ...
    parser.add_argument('--train_raw_path', default='data/train_with_summ.txt', type=str, required=False, help='原始训练语料')
...
def preprocess_mmi_raw_data(args, tokenizer, n_ctx):
    ...
    with open(args.train_mmi_tokenized_path, "w", encoding="utf-8") as f:
        for dialogue_index, dialogue in enumerate(tqdm(train_data)):
            utterances = [dialogue['summarization'], dialogue['article'], ]
            dialogue_ids = [tokenizer.cls_token_id]  # 每个dialogue以[CLS]开头
            for utterance in utterances:
                dialogue_ids.extend([tokenizer.convert_tokens_to_ids(word) for word in utterance])
                dialogue_ids.append(tokenizer.sep_token_id)  # 每个utterance之后添加[SEP]，表示utterance结束
            ...
            if dialogue_index < len(train_data) - 1:
                f.write("\n")
    logger.info("finish preprocessing raw data,the result is stored in {}".format(args.train_mmi_tokenized_path))
...
''' 说明书-这整个项目原本是一个闲聊对话的项目，原作者预训练了2个GPT-2模型 '''
''' 一个是dialogue_model,另一个是mmi_model,这个mmi是作者复现一顶会论文后做的优化，可以忽略，与我们的文本生成无关'''
'''我们要做的就是针对文本生成进行精调，也就是train.py这部分代码，我使用2块p40训练了大约10小时左右。'''
'''精调完成后，运行interacter2.py 进行人机交互，观看文本生成效果'''
